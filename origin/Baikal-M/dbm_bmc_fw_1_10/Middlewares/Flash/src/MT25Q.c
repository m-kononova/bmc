...
ReturnType Driver_Init(FLASH_DEVICE_OBJECT *flash_device_object)
{
	...
	FlashReadDeviceIdentification(&deviceFullId[0]);
	device |= deviceFullId[0];
	device <<= 8;
	device |= deviceFullId[1];
	device <<= 8;
	device |= deviceFullId[2];
//	device <<= 8; // We do not use the fourth byte of ID
//	device |= deviceFullId[3];
	if ((device & MEM_TYPE_MASK) == MEM_TYPE_MICRON) {
		fdo->Desc.FlashId = device;
		memcpy(fdo->Desc.FlashUniqueId, &deviceFullId[4], 17);
		...
		fdo->Desc.FlashSize = (*((uint32*)&buffer[tableAddress + DTABLE1_FLASH_SIZE])) / 8;
		fdo->Desc.AvalAddrMode = BYTE_TO_MODE((*((uint8*)&buffer[tableAddress + DTABLE1_FLASH_ADDR_MODE_BYTE])));
...
#undef ADDR_MODE_AUTO_DETECT
#ifdef ADDR_MODE_AUTO_DETECT
		if (fdo->Desc.AvalAddrMode == FLASH_ONLY_4_BYTE_ADDR_MODE)
		{
		...
		else if ((fdo->Desc.FlashSize > SIZE_16MB) && (fdo->Desc.AvalAddrMode == FLASH_3_OR_4_BYTE_ADDR_MODE))
		{
			fdo->GenOp.Enter4ByteAddressMode();
			/* verify current addr mode */
			fdo->GenOp.ReadFlagStatusRegister(&flag);
...
*******************************************************************************/
ReturnType FlashReadDeviceIdentification(uint8 *uwpDeviceIdentification)
{
	// uint8  cRDID = SPI_FLASH_INS_RDID;
	// uint8  pIdentification[20];

	// READ ID CMD: SPI_FLASH_INS_RDID = 0x9F/0x9E
	flashReadId(uwpDeviceIdentification);

	// Step 3: Device Identification is returned (manufacturer id + memory type + memory capacity)
//	*uwpDeviceIdentification = pIdentification[0];
//	*uwpDeviceIdentification <<= 8;
//	*uwpDeviceIdentification |= pIdentification[1];
//	*uwpDeviceIdentification <<= 8;
...
ReturnType  FlashWriteStatusRegister(uint8 ucStatusRegister)
{
	...
	// Step 4: Wait until the operation completes or a tiSerialize_SPI_DeInit();meout occurs.
...
//	{
	...
	//if (insInstruction == 0x5A)
	// {
	//	flashRead(char_stream_recv.pChar, char_stream_recv.length, udAddr, insInstruction);
	// }
	// else
	// {
...
//
//	cmd[0] = insInstruction; // FOUR BYTE PAGE PROGRAMM
//	if ((fdo->Desc.NumAddrByte == FLASH_3_BYTE_ADDR_MODE) || (insInstruction == 0x5A)) // 0x5A - read serial discovery parameter
//	{
//		sendLength = sizeof(cmd) - 1;
//		cmd[1] = (uint8_t) (udAddr >> 16);
//		cmd[2] = (uint8_t) (udAddr >> 8);
//		cmd[3] = (uint8_t) (udAddr);
//	}
//	else
//	{
//		sendLength = sizeof(cmd);
//		cmd[1] = (uint8_t) (udAddr >> 24);
//		cmd[2] = (uint8_t) (udAddr >> 16);
//		cmd[3] = (uint8_t) (udAddr >> 8);
//		cmd[4] = (uint8_t) (udAddr);
//	}

	uint8_t cmd[5];
	uint16_t cmdLen = 0;
	cmdLen = prepareCmdVector(cmd, insInstruction, udAddr, fdo->Desc.NumAddrByte);


	// memset(tmpBuff_my, 0, 258);

	NN_SpiSSEnable(B_TRUE);
		writeData(cmd, cmdLen, B_FALSE);
		readData(ucpElements, udNrOfElementsToRead, B_FALSE);
	NN_SpiSSDisable(B_TRUE);

	// memcpy(tmpBuff_my, ucpElements, udNrOfElementsToRead);
	return Flash_Success;
}
...
*******************************************************************************/
//uint32_t adrSave[4096];
//uint32_t adrLen[4096];
                           ...
                           uint32 udNrOfElementsInArray, uint8 ubSpiInstruction)
{
	...
	uint8_t cmd[5];
	uint16_t cmdLen = 0;
	cmdLen = prepareCmdVector(cmd, SPI_FLASH_INS_PP, udAddr, fdo->Desc.NumAddrByte);

	writeEnable();
	NN_SpiSSEnable(B_TRUE);
		writeData(cmd, cmdLen, B_FALSE);
		//writeData(char_stream_send.pChar, char_stream_send.length - 4, B_FALSE);
		writeData(sendCommandInBuff, udNrOfElementsInArray, B_FALSE);
	NN_SpiSSDisable(B_TRUE);
...
ReturnType  FlashSectorErase(uSectorType uscSectorNr)
{
	...
	uscSectorNr *= fdo->Desc.FlashSectorSize; // 0x10000 = (65536d) - Is default value for sector;
...
//	uint16_t cmdLen = 0;
//
//	cmd[0] = fdo->Desc.FlashSectorEraseCmd; // 0xD8 - by default
//
//	if (fdo->Desc.NumAddrByte == FLASH_4_BYTE_ADDR_MODE)
//	{
//		cmdLen = sizeof(cmd);
//		cmd[1] = (uint8_t)(uscSectorNr >> 24);
//		cmd[2] = (uint8_t)(uscSectorNr >> 16);
//		cmd[3] = (uint8_t)(uscSectorNr >> 8);
//		cmd[4] = (uint8_t)(uscSectorNr);
//	}
//	else
//	{
//		cmdLen = sizeof(cmd) - 1;
//		cmd[1] = (uint8_t)(uscSectorNr >> 16);
//		cmd[2] = (uint8_t)(uscSectorNr >> 8);
//		cmd[3] = (uint8_t)(uscSectorNr);
//	}

	uint8_t cmd[5];
	uint16_t cmdLen = 0;
	cmdLen = prepareCmdVector(cmd, fdo->Desc.FlashSectorEraseCmd, uscSectorNr, fdo->Desc.NumAddrByte);

	writeEnable();
	NN_SpiSSEnable(B_TRUE);
		writeData(cmd, cmdLen, B_FALSE);
	NN_SpiSSDisable(B_TRUE);
	// eraseSector(uscSectorNr);
	while(readStatus() == S_BUSY){};
	// fourByteAddressing(AM_DISABLE);
...
ReturnType  FlashReadFlagStatusRegister(uint8 *ucpFlagStatusRegister)
{
	...
	uint8_t res = readStatusFlags();
	if ((res & 0x01) == 0x01)
	{
		// 0x01 - 4 bytes mode is used
		(*ucpFlagStatusRegister) = 1;
	}
	else
	{
		// 0x00 - 3 bytes mode is used
		(*ucpFlagStatusRegister) = 0;
	}
	//Serialize_SPI_Init();
